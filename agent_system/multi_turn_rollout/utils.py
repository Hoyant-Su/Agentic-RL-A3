...
def adjust_batch(config, data: DataProto, mode="copy") -> DataProto:
    ...
    size_divisor_rollout = (
        config.actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu * world_size
    )
    if config.algorithm.use_kl_in_reward or config.actor_rollout_ref.actor.use_kl_loss:
        size_divisor_ref = (
            config.actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu * world_size
        )
    else:
    ...
    if "multi_modal_inputs" in data.non_tensor_batch:
        ...
        size_divisor_actor = (
            config.actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu * world_size
        )
    size_divisor = np.lcm.reduce(
        np.array([size_divisor_ref, size_divisor_rollout, size_divisor_actor])
    ).item()
    bs = int(len(data))
    if bs <= 0:
        raise ValueError(
            "adjust_batch received an empty batch (bs=0). Check dataloader / dataset filtering."
        )
    remainder = int(bs % size_divisor)
    if remainder == 0:
        return data
    if mode == "delete":
        ...
        non_tensor_data = {
            key: val[keep_mask] for key, val in data.non_tensor_batch.items()
        }
        ...
        to_add = int(size_divisor - remainder)
        replace = to_add > bs
        try:
            dup_indices = np.random.choice(bs, to_add, replace=replace)
        except ValueError:
