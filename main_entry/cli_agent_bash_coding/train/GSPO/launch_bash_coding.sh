...
if [ -n "${REQUESTED_CHECKPOINT_PAIR:-}" ] && [ -f "$MERGE_SOURCE_CONFIG_YAML" ] && [ -f "${CONFIG_YAML:-}" ]; then
    ...
    if [ -n "${BASH_CODING_RESULTS_NESTED_SUBDIR:-}" ]; then
        echo "GSPO: results subdir: ${BASH_CODING_RESULTS_NESTED_SUBDIR} (harness+env_max_steps+subdir naming from train config; merged save config patches env from train)"
    fi
fi
