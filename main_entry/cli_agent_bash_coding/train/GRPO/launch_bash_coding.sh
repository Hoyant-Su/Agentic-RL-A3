...
if [ -n "${REQUESTED_CHECKPOINT_PAIR:-}" ] && [ -f "$MERGE_SOURCE_CONFIG_YAML" ] && [ -f "${CONFIG_YAML:-}" ]; then
    ...
    if [ -n "${BASH_CODING_RESULTS_NESTED_SUBDIR:-}" ]; then
        echo "GRPO: results subdir: ${BASH_CODING_RESULTS_NESTED_SUBDIR} (train: bench+harness+steps; save config = results timestamp + env from train)"
    fi
fi
...
if [ "$DISABLE_CUDA_GRAPH" = "1" ]; then
    ...
    CUDA_GRAPH_MAX_BS="${CUDA_GRAPH_MAX_BS:-$cuda_graph_max_bs}"
fi
