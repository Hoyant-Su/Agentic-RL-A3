...
export BASH_CODING_CONFIG_YAML="$(resolve_abs_path "$CONFIG_YAML")"
...
EXTRA_CLIP_OVERRIDES=()
[[ -n "${actor_clip_ratio:-}" ]] && EXTRA_CLIP_OVERRIDES+=("++actor_rollout_ref.actor.clip_ratio=${actor_clip_ratio}")
[[ -n "${actor_clip_ratio_low:-}" ]] && EXTRA_CLIP_OVERRIDES+=("++actor_rollout_ref.actor.clip_ratio_low=${actor_clip_ratio_low}")
[[ -n "${actor_clip_ratio_high:-}" ]] && EXTRA_CLIP_OVERRIDES+=("++actor_rollout_ref.actor.clip_ratio_high=${actor_clip_ratio_high}")
[[ -n "${actor_grad_clip:-}" ]] && EXTRA_CLIP_OVERRIDES+=("++actor_rollout_ref.actor.grad_clip=${actor_grad_clip}")
[[ -n "${actor_entropy_coef:-}" ]] && EXTRA_CLIP_OVERRIDES+=("++actor_rollout_ref.actor.entropy_coeff=${actor_entropy_coef}")
...
python3 -m "$ENTRYPOINT" \
    ...
    ${EXTRA_CLIP_OVERRIDES[@]+"${EXTRA_CLIP_OVERRIDES[@]}"} \
