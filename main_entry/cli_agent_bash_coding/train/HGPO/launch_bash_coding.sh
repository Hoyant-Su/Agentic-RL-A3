...
REQUESTED_RESUME_FOR_INFERENCE="${RESUME_FOR_INFERENCE:-${resume_for_inference:-}}"
...
find_latest_result_step() {
    python3 - <<'PY' "$1"
from pathlib import Path
import sys

result_dir = Path(sys.argv[1])
steps = sorted(int(path.stem) for path in result_dir.glob("*.jsonl") if path.stem.isdigit())
if steps:
    print(steps[-1])
PY
}

RESUME_INFERENCE_DIR=""
if [ -n "$REQUESTED_RESUME_FOR_INFERENCE" ]; then
    RESUME_INFERENCE_DIR="$(resolve_result_dir "$REQUESTED_RESUME_FOR_INFERENCE")"
    CONFIG_YAML="$RESUME_INFERENCE_DIR/config.yaml"
    if [ ! -f "$CONFIG_YAML" ]; then
        echo "ERROR: Missing inference resume config: $CONFIG_YAML"
        exit 1
    fi
    eval "$(python3 "$PROJECT_DIR/main_entry/cli_agent_bash_coding/train/load_config_yaml.py" "$CONFIG_YAML")"
fi

if [ -n "$REQUESTED_RESUME_FOR_INFERENCE" ] && [ -z "$REQUESTED_CHECKPOINT_PAIR" ] && [ -z "$REQUESTED_RESUME_FOR_TRAINING" ] && [ "$enable_pretrained_eval_only" != "1" ]; then
    RESUME_STEP="$(find_latest_result_step "$RESUME_INFERENCE_DIR")"
    if [ -z "$RESUME_STEP" ]; then
        echo "ERROR: Cannot infer checkpoint step from results directory: $RESUME_INFERENCE_DIR"
        exit 1
    fi
    REQUESTED_CHECKPOINT_PAIR="$PROJECT_DIR/main_entry/cli_agent_bash_coding/checkpoints/$(basename "$(dirname "$RESUME_INFERENCE_DIR")")/$(basename "$RESUME_INFERENCE_DIR")/global_step_${RESUME_STEP}:$pretrained_model_path_default"
fi
...
if [ -n "$CHECKPOINT_SOURCE_PATH" ]; then
    ...
    resume_for_inference="$REQUESTED_RESUME_FOR_INFERENCE"
fi
...
if [ -n "${REQUESTED_CHECKPOINT_PAIR:-}" ] && [ -f "$MERGE_SOURCE_CONFIG_YAML" ] && [ -f "${CONFIG_YAML:-}" ]; then
    ...
    if [ -n "${BASH_CODING_RESULTS_NESTED_SUBDIR:-}" ]; then
        echo "HGPO: results subdir: ${BASH_CODING_RESULTS_NESTED_SUBDIR} (harness+env_max_steps+subdir from train; merged config = results timestamp + env overrides from train)"
    fi
fi
...
if [ -n "$RESUME_INFERENCE_DIR" ]; then
    save_dir="$RESUME_INFERENCE_DIR"
    run_id="$(basename "$RESUME_INFERENCE_DIR")"
fi
if [ -n "${BASH_CODING_RESULTS_NESTED_SUBDIR:-}" ]; then
...
if [ -n "$RESUME_INFERENCE_DIR" ] && [ "$IS_VAL_ONLY" = "0" ]; then
    echo "ERROR: resume_for_inference only supports TEST mode or PRETRAINED EVAL-ONLY mode"
    exit 1
fi

DISABLE_CUDA_GRAPH="${DISABLE_CUDA_GRAPH:-$DEFAULT_DISABLE_CUDA_GRAPH}"
...
if [ "${disable_qwen_thinking:-1}" = "1" ]; then
...
NO_PROGRESS_ON_ANSWER="${no_progress_on_answer:-1}"
...
HGPO_MODE="$mode"
HGPO_ENABLE_SIMILARITY_SM="$enable_similarity_sequence_matcher"
HGPO_ENABLE_SIMILARITY_LLM="$enable_similarity_llm"
HGPO_SIMILARITY_THRESH="$similarity_thresh"
HGPO_SIMILARITY_BATCH_SIZE="$similarity_batch_size"
HGPO_LENGTH_WEIGHT_ALPHA="$length_weight_alpha"
HGPO_BASE_GROUP="$base_group"
[ "$HGPO_BASE_GROUP" = "1" ] && HGPO_BASE_GROUP=True || HGPO_BASE_GROUP=False
HGPO_EPSILON="$epsilon"
[ "$HGPO_ENABLE_SIMILARITY_SM" = "1" ] && HGPO_ENABLE_SIMILARITY_SM=True || HGPO_ENABLE_SIMILARITY_SM=False
[ "$HGPO_ENABLE_SIMILARITY_LLM" = "1" ] && HGPO_ENABLE_SIMILARITY_LLM=True || HGPO_ENABLE_SIMILARITY_LLM=False
...
python3 -m "$ENTRYPOINT" \
    ...
    algorithm.hgpo.mode=$HGPO_MODE \
    algorithm.hgpo.length_weight_alpha=$HGPO_LENGTH_WEIGHT_ALPHA \
    algorithm.hgpo.base_group=$HGPO_BASE_GROUP \
    algorithm.hgpo.epsilon=$HGPO_EPSILON \
    algorithm.hgpo.enable_similarity_sequence_matcher=$HGPO_ENABLE_SIMILARITY_SM \
    algorithm.hgpo.enable_similarity_llm=$HGPO_ENABLE_SIMILARITY_LLM \
    algorithm.hgpo.similarity_thresh=$HGPO_SIMILARITY_THRESH \
    algorithm.hgpo.similarity_batch_size=$HGPO_SIMILARITY_BATCH_SIZE \
