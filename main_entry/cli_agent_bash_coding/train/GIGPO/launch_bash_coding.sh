...
if [ -n "${REQUESTED_CHECKPOINT_PAIR:-}" ] && [ -f "$MERGE_SOURCE_CONFIG_YAML" ] && [ -f "${CONFIG_YAML:-}" ]; then
    ...
    if [ -n "${BASH_CODING_RESULTS_NESTED_SUBDIR:-}" ]; then
        echo "GIGPO: results subdir: ${BASH_CODING_RESULTS_NESTED_SUBDIR} (harness+env_max_steps+subdir naming from train config; other hyperparams from results timestamp config when merged)"
    fi
fi
...
if [ "$RETROAGENT_ENABLE" = "1" ]; then
    RETROAGENT_MEMORY_PATH="$(resolve_abs_path "$tensorboard_dir")/memory_store.json"
fi
...
GIGPO_MODE="$mode"
GIGPO_ENABLE_SIMILARITY_SM="$enable_similarity_sequence_matcher"
GIGPO_ENABLE_SIMILARITY_LLM="$enable_similarity_llm"
[ "$GIGPO_ENABLE_SIMILARITY_SM" = "1" ] && GIGPO_ENABLE_SIMILARITY_SM=True || GIGPO_ENABLE_SIMILARITY_SM=False
[ "$GIGPO_ENABLE_SIMILARITY_LLM" = "1" ] && GIGPO_ENABLE_SIMILARITY_LLM=True || GIGPO_ENABLE_SIMILARITY_LLM=False
GIGPO_SIMILARITY_THRESH="$similarity_thresh"
...
python3 -m "$ENTRYPOINT" \
    ...
    algorithm.gigpo.enable_similarity_sequence_matcher=$GIGPO_ENABLE_SIMILARITY_SM \
    algorithm.gigpo.enable_similarity_llm=$GIGPO_ENABLE_SIMILARITY_LLM \
