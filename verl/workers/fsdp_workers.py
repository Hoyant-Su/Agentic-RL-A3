...
from verl.utils import hf_processor, hf_tokenizer, resize_model_embeddings_to_tokenizer
...
from main_entry.cli_agent_bash_coding.algo.registry import (
    get_bash_coding_algorithm,
    list_bash_coding_algorithms,
)
...
def _get_enabled_bash_coding_algorithm(config):
    enabled_name = None
    for name in list_bash_coding_algorithms():
        if not _is_enabled_flag(config.get(f"enable_{name}", 0)):
            continue
        if enabled_name is not None and enabled_name != name:
            raise ValueError(
                f"Multiple bash coding algorithms are enabled: {enabled_name}, {name}"
            )
        enabled_name = name
    if enabled_name is not None:
        return get_bash_coding_algorithm(enabled_name)
    algorithm_name = str(config.get("algorithm_name", "")).strip().lower()
    if algorithm_name == "":
        return None
    return get_bash_coding_algorithm(algorithm_name)

def _inject_algorithm_hparams(func, config, kwargs):
    merged = dict(kwargs)
    for name in inspect.signature(func).parameters:
        if name in merged:
            continue
        value = config.get(f"algorithm_{name}", None)
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            resize_model_embeddings_to_tokenizer(actor_module, self.tokenizer)
            if use_liger:
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            algorithm = _get_enabled_bash_coding_algorithm(self.config.actor)
            if algorithm is not None and algorithm.compute_advantage is not None:
                adv_base = {
                    "data": data,
                    "multi_turn": self.config.rollout.multi_turn.enable,
                }
                if (
                    "tokenizer"
                    in inspect.signature(algorithm.compute_advantage).parameters
                ):
                    adv_base["tokenizer"] = (
                ...
                advantage_kwargs = _inject_algorithm_hparams(
                    algorithm.compute_advantage,
                    self.config.actor,
                    adv_base,
                )
                data = algorithm.compute_advantage(**advantage_kwargs)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            resize_model_embeddings_to_tokenizer(critic_module, self.tokenizer)
...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            resize_model_embeddings_to_tokenizer(reward_module, self.tokenizer)
            apply_monkey_patch(
                model=reward_module,
                use_remove_padding=config.model.get("use_remove_padding", False),
                ulysses_sp_size=self.ulysses_sequence_parallel_size,
