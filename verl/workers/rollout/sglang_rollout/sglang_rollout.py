...
from verl.utils.bash_coding import is_bash_coding_enabled
...
REQ_CONSTRAINTS_KEY = "__constrained_decoding__"
...
def _extract_constrained_decoding(meta_info: dict) -> dict:
    if not is_bash_coding_enabled():
        return {}
    constrained_keys = ("json_schema", "regex", "ebnf", "structural_tag")
    return {
        key: meta_info[key]
...
class SGLangRollout(BaseRollout):
    ...
    def _init_distributed_env(self, device_mesh_cpu, **kwargs):
        ...
        if self._rank == 0:
            logger.warning(
                f"[SGLangRollout] After Ray assignment, CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}"
            )
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            sglang_engine_kwargs = engine_kwargs.get("sglang", {})
            ...
            disable_cuda_graph = self.config.get("disable_cuda_graph", False)
            disable_cuda_graph_padding = self.config.get(
                "disable_cuda_graph_padding", False
            )
            cuda_graph_max_bs = self.config.get("cuda_graph_max_bs", None)
            self._engine = Engine(
    ...
    def _init_sampling_params(self, **kwargs):
        ...
        for k in self.config.keys():
            if hasattr(SamplingParams(), str(k)) or (
                is_bash_coding_enabled() and str(k) == "stop"
            ):
                kwargs[k] = self.config.get(k)
        self.sampling_params = kwargs

    @staticmethod
    def _extract_constrained_decoding_kwargs(meta_info) -> dict:
        constrained_keys = ("json_schema", "regex", "ebnf", "structural_tag")
        merged = {}
        for key in constrained_keys:
            if key in meta_info:
                merged[key] = meta_info[key]
        return merged
    ...
    def update_sampling_params(self, **kwargs):
        ...
        for key in kwargs:
            if key in self.sampling_params:
                old_sampling_params_args[key] = self.sampling_params[key]
            else:
    ...
    def _batch_level_generate_sequences(
        ...
        kwargs.update(self._extract_constrained_decoding_kwargs(prompts.meta_info))
        with self.update_sampling_params(**kwargs):
            if self._tp_rank == 0:
                loop = (
                    get_event_loop()
                    if is_bash_coding_enabled()
    ...
    async def _handle_engine_call(
        ...
        if is_bash_coding_enabled():
            constrained_decoding = dict(_req.tools_kwargs.get(REQ_CONSTRAINTS_KEY, {}))
            kwargs.update(
                self._extract_constrained_decoding_kwargs(constrained_decoding)
            )
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                if self._tool_schemas:
                    ...
                    _tool_schemas = []
                    _tools_kwargs = {}
                constrained_decoding = _extract_constrained_decoding(prompts.meta_info)
                if constrained_decoding:
                    _tools_kwargs = dict(_tools_kwargs)
                    _tools_kwargs[REQ_CONSTRAINTS_KEY] = deepcopy(constrained_decoding)
                req = AsyncRolloutRequest(
    ...
    def execute_method(self, method: Union[str, bytes], *args, **kwargs):
        if method == "chat_completion":
            ...
            sampling_params_dict = {
                ...
                "max_new_tokens": json_request.get(
