...
class AsyncRolloutRequest(BaseModel):
    ...
    def add_assistant_message(
        ...
        if format in self.format_config:
            ...
            if tool_calls is not None:
                content = content_with_tool_calls.split(f"{prefix_msg}")[-1].split(
                    f"{suffix_msg}"
                )[0]
            ...
            _delta_position_ids = compute_position_id_with_mask(
                torch.tensor(_attention_mask)
            ).tolist()
            last_position_id = self.position_ids[-1]
            _position_ids = [
                pos_id + last_position_id for pos_id in _delta_position_ids
            ]
    ...
    def add_tool_response_message(
        ...
        self.messages.append(msg)
        if format in self.format_config:
            merge_tool_responses = self.format_config[format].get(
                "merge_tool_response", False
            )
            ...
            prefix_resp_token_ids = tokenizer.encode(
                prefix_resp, add_special_tokens=False
            )
            suffix_resp = self.format_config[format].get("tool_response_suffix_msg", "")
            suffix_resp_token_ids = tokenizer.encode(
                suffix_resp, add_special_tokens=False
            )
            ...
            if (
                ...
                or self.input_ids[-len(suffix_resp_token_ids) :]
                == suffix_resp_token_ids
            ):
                append_token_ids = (
                    prefix_resp_token_ids + content_token_ids + full_suffix_token_ids
                )
