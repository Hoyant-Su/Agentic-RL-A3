...
from verl.utils import hf_tokenizer, resize_model_embeddings_to_tokenizer
...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        if self.config.ulysses_sequence_parallel_size > 1:
            assert self.use_remove_padding, (
        ...
        with init_context():
            ...
            resize_model_embeddings_to_tokenizer(self.model, self.tokenizer)
            if (
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        enable_sft_modified = bool(
            int(getattr(self.config.trainer, "enable_sft_modified", 0))
        )
        negative_mask = None
        illegal_token_ids = None
        unlikelihood_coef = 0.0
        if enable_sft_modified:
            negative_mask = (
                batch.pop("negative_mask") if "negative_mask" in batch.keys() else None
            )
            illegal_token_ids = (
                batch.pop("illegal_token_ids")
                if "illegal_token_ids" in batch.keys()
                else None
            )
            if negative_mask is not None:
                negative_mask = negative_mask[:, :-1].reshape(-1).to(self.device_name)
            if illegal_token_ids is not None:
                illegal_token_ids = illegal_token_ids[0].to(self.device_name)
            unlikelihood_coef = float(
                getattr(self.config.trainer, "illegal_tag_unlikelihood_coef", 0.0)
            )
        ...
        with (
            ...
            if not use_sp:
                ...
                if (
                    negative_mask is not None
                    and illegal_token_ids is not None
                    and unlikelihood_coef > 0
                ):
                    illegal_probs = (
                        torch.softmax(shift_logits, dim=-1)
                        .index_select(dim=-1, index=illegal_token_ids)
                        .sum(dim=-1)
                    )
                    illegal_probs = illegal_probs.clamp(max=1 - 1e-6)
                    negative_loss = -torch.log1p(-illegal_probs)
                    loss = loss + unlikelihood_coef * negative_loss * negative_mask.to(
                        loss.device
                    )
            else:
